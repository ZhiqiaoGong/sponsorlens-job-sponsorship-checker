(function sponsorLensAnalyzerFactory(root, factory) {
  ...
  const RULES = [
    ...
    },
    {
      ...
      pattern: /\b(?:(?:authorized|eligible|permitted)\s+to\s+work|(?:(?:continuous|current|ongoing|unrestricted)\s+)?work\s+authori[sz]ation)\b[\s\S]{0,120}\bwithout\b[\s\S]{0,60}\bsponsorships?\b|\bwithout\b[\s\S]{0,45}\bsponsorships?\b[\s\S]{0,120}\b(?:now|currently)\b[\s\S]{0,35}\b(?:future|later)\b/gi
    },
    {
    ...
    {
      id: "student_work_authorization",
      category: "review",
      confidence: "medium",
      title: "Student work authorization requirement",
      pattern: /\b(?:F-?1(?:\s+visa)?\s+students?|international\s+students?)\b[\s\S]{0,220}?\b(?:CPT|OPT|Curricular\s+Practical\s+Training|Optional\s+Practical\s+Training)\b|\b(?:CPT(?:\s*\/\s*OPT)?|OPT|Curricular\s+Practical\s+Training|Optional\s+Practical\s+Training)\b.{0,180}\b(?:approved|authori[sz]ed|eligible|degree|university|school|employment\s+authori[sz]ation)\b/gi
    },
    {
      id: "us_person_export",
      category: "review",
      ...
      // The named export-control regimes get a wide window because listings
      // routinely spell out who counts as a U.S. person — citizen, permanent
      // resident, protected individual — before naming ITAR or the EAR. The
      // bare words "export" and "controlled" keep the narrow window, since on
      // their own they appear in unrelated sentences.
      pattern: /\b(?:U\.?\s*S\.?|United\s+States)\s+persons?\b[\s\S]{0,240}\b(?:ITAR|EAR|export[\s-](?:control(?:led|s)?|licen[sc]es?|regulations?)|International\s+Traffic\s+in\s+Arms|Export\s+Administration\s+Regulations)\b|\b(?:U\.?\s*S\.?|United\s+States)\s+persons?\b.{0,100}\b(?:export|control(?:led)?)\b|\b(?:export|ITAR|EAR)\b.{0,100}\b(?:U\.?\s*S\.?|United\s+States)\s+persons?\b|\b(?:U\.?\s*S\.?|United\s+States)\s+persons?\s+status\b[^\n.!?]{0,60}\brequired\b|\bmust\s+be\s+(?:an?\s+)?(?:U\.?\s*S\.?|United\s+States)\s+person\b|\b(?:U\.?\s*S\.?|United\s+States)\s+government\s+export\s+regulations?\b[\s\S]{0,900}?\beligible\s+to\s+obtain\s+(?:the\s+)?required\s+authorizations?\s+from\s+(?:the\s+)?(?:U\.?\s*S\.?|United\s+States)\s+department\s+of\s+state\b/gi
    },
    {
      id: "export_authorization_condition",
      category: "review",
      confidence: "medium",
      title: "Offer depends on export-control authorization",
      pattern: /\b(?:offers?\s+for\s+employment|employment\s+offers?)\b[\s\S]{0,70}\b(?:conditioned|contingent|dependent)\s+on\b[\s\S]{0,90}\bauthori[sz]ation\b[\s\S]{0,220}\b(?:export\s+control|export\s+license|Export\s+Administration\s+Regulations|International\s+Traffic\s+in\s+Arms\s+Regulations|EAR|ITAR)\b/gi
    },
    {
  ...
  function isEqualOpportunityAuthorizationAside(rule, match, text) {
    if (rule.id !== "current_authorization") return false;
    const paragraphStart = text.lastIndexOf("\n\n", match.index);
    const paragraphEnd = text.indexOf("\n\n", match.index + match[0].length);
    const paragraph = text.slice(
      paragraphStart < 0 ? Math.max(0, match.index - 1600) : paragraphStart + 2,
      paragraphEnd < 0 ? Math.min(text.length, match.index + 1600) : paragraphEnd
    );
    return /\bequal\s+(?:employment\s+)?opportunity\s+employer\b/i.test(paragraph) &&
      /\bemployment\s+decisions?\b/i.test(paragraph) &&
      /\b(?:race|color|national\s+origin|religion|citizenship\s+status)\b/i.test(paragraph);
  }
  ...
  function runRule(rule, text, evidence, seen) {
    ...
    while ((match = regex.exec(text)) !== null && sequence < 8) {
      if (isEqualOpportunityAuthorizationAside(rule, match, text)) {
        sequence += 1;
  ...
  function assessJobLikelihood(text, meta) {
    ...
    // An embedded applicant-tracking board keeps the company's own careers path
    // and names the listing in a query parameter, so the parameter is the only
    // part of the URL that separates one listing from the board's index page.
    if (/(?:\/jobs?\/|\/careers?\/|jobid=|job_id=|gh_jid=|reqid=|postingid=|positionid=|viewjob|requisition|myworkdayjobs|greenhouse\.io|lever\.co|ashbyhq\.com|smartrecruiters|linkedin\.com\/jobs)/.test(url)) {
      score += 2;
    }
    ...
    const signals = [
      ...
      // Plain-language headings that replace the classic ones on many boards.
      /\bwhat you(?:'|’)?(?:ll| will) be doing\b/,
      /\bwhat you(?:'|’)?(?:ll| will) do\b/,
      /\bwhat we(?:'|’)?(?:re| are) looking for\b/,
      /\babout (?:the|this) (?:position|job|opportunity)\b/,
      /\bwho you are\b/
    ];
