(function sponsorLensAnalyzerFactory(root, factory) {
  ...
  const RULES = [
    ...
    },
    {
      ...
      pattern: /\b(?:only|limited\s+to)\s+(?:(?:U\.?\s*S\.?|United\s+States)\s+)?citizens?(?:\s+(?:or|and)\s+(?:lawful\s+)?permanent\s+residents?)?\b|\b(?:U\.?\s*S\.?\s+citizens?\s+(?:or|and)\s+)?(?:green\s+card\s+holders?|(?:lawful\s+)?permanent\s+residents?)\s+only\b/gi
    },
    {
      id: "conditional_only",
    ...
    },
    {
      ...
      pattern: /\b(?:U\.?\s*S\.?|United\s+States)\s+persons?\b.{0,100}\b(?:export|ITAR|EAR|control(?:led)?)\b|\b(?:export|ITAR|EAR)\b.{0,100}\b(?:U\.?\s*S\.?|United\s+States)\s+persons?\b|\bmust\s+be\s+(?:an?\s+)?(?:U\.?\s*S\.?|United\s+States)\s+person\b/gi
    },
    {
      ...
      pattern: /\b(?:must\s+(?:hold|have|obtain)|ability\s+to\s+obtain|eligible\s+for)\b.{0,65}\b(?:security|government)\s+clearance\b|\b(?:active\s+)?(?:secret|top\s+secret|TS\/SCI|security)\s+clearance\s+(?:is\s+)?required\b/gi
    }
  ];
