(function sponsorLensLocalModelPolicyFactory(root, factory) {
  ...
  function isHeadingLike(value) {
    ...
    if (!/\b(?:requirements?|eligibility|authorization|visa|sponsorship|citizenship|export|clearance|immigration)\b/i.test(text)) {
      return false;
    }
    const connectors = new Set(["and", "for", "of", "or", "the", "to"]);
    return words.every((word) => {
      const clean = word.replace(/^[-/]+|[-/]+$/g, "");
      return connectors.has(clean.toLowerCase()) ||
        /^[A-Z]/.test(clean) ||
        /^[A-Z0-9/-]+$/.test(clean);
    });
  }

  function findHeadingContextEnd(text, boundaryEnd, maximum) {
    let cursor = boundaryEnd;
    let end = boundaryEnd;
    let blocks = 0;

    while (cursor < maximum && blocks < 2) {
      while (cursor < maximum && /\s/.test(text[cursor])) cursor += 1;
      if (cursor >= maximum) break;
      const blockStart = cursor;
      let blockEnd = text.indexOf("\n", blockStart);
      if (blockEnd < 0 || blockEnd > maximum) blockEnd = maximum;
      const blockText = text.slice(blockStart, blockEnd).trim();
      if (!blockText) {
        cursor = blockEnd + 1;
        continue;
      }
      if (isHeadingLike(blockText)) break;

      blocks += 1;
      end = blockEnd;
      for (let index = blockStart; index < blockEnd; index += 1) {
        if (isBoundaryAt(text, index) && text[index] !== "\n" && text[index] !== "•") {
          return index + 1;
        }
      }
      cursor = blockEnd + 1;
  ...
  function findRightBoundary(text, signalEnd, windowStart) {
    const maximum = Math.min(text.length, windowStart + MAX_WINDOW_LENGTH);
    for (let index = signalEnd; index < maximum; index += 1) {
      if (!isBoundaryAt(text, index)) continue;
      const boundaryEnd = index + 1;
      if (
        text[index] === "\n" &&
        isHeadingLike(text.slice(windowStart, index))
      ) {
        return findHeadingContextEnd(text, boundaryEnd, maximum);
      }
      return boundaryEnd;
    }
    return maximum;
  ...
  function extractCandidateWindows(rawText, options) {
    ...
    while ((match = regex.exec(text)) !== null) {
      const windowStart = findLeftBoundary(text, match.index);
      ...
      if (overlapping) {
        if (window.start <= overlapping.index && window.end >= overlapping.end) {
          overlapping.id = `window:${window.start}:${hashValue(window.text)}`;
          overlapping.text = window.text;
          overlapping.index = window.start;
          overlapping.end = window.end;
        }
